...
window.onload = function() {
	...
	//新游戏
	addHandler(newGame, "touchstart", function() {
    	hideBox();
    	anotherGame();
    });
    addHandler(tryAgain, "touchstart", function() {
    	hideBox();
    	anotherGame();
    });
...
function createNew(a) {
	...
	switch(temp) {
		case 0:
			newDiv.style.left = 4*2+20*3+"vw";
			break;
		default:
			newDiv.style.left = (temp)*2+20*(temp-1)+"vw";
			break;
	}
	newDiv.style.top = differ*2+20*(differ-1)+"vw";
...
function addElem(elem) {
	elem.innerHTML = parseInt(elem.innerHTML, 10) * 2;
	elem.style.backgroundColor = colorInfo.bgColor[elem.innerHTML + ''];
	var n = parseInt(elem.innerHTML, 10);
	if(n > 4) {
	...
	//计分
	var score = document.querySelector("#score");
	var num = parseInt(score.innerHTML, 10);
